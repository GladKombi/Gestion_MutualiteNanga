...
require_once('../models/select/select-Paiment.php');
?>
<!DOCTYPE html>
<html lang="fr">
...
<body class="bg-white dark:bg-gray-900 text-gray-800 dark:text-white flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col">
        ...
        <header class="flex items-center justify-between bg-white dark:bg-gray-800 shadow px-4 py-3">
            <h2 class="text-lg font-semibold text-primary">Paiement</h2>
        ...
        <main class="p-4 overflow-auto">
            ...
            <button onclick="openModal('modalAdhesion')" class="mb-4 px-4 py-2 bg-primary text-white rounded"> Paiement cotisation</button>
            <?php
                    ...
                    <div class="bg-white dark:bg-gray-800 p-6 rounded shadow mb-6">
                        ...
                        <form action="<?= $url ?>" method="POST" enctype="multipart/form-data">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    <label class="block mb-1">Cotisation <span class="text-red-600"> *</span></label>
                                    <select name="cotisation" required class="w-full px-4 py-2 border rounded dark:bg-gray-700 dark:text-white">
                                        <?php while ($cotisation = $getCotisaion->fetch()) {
                                            if (isset($_GET["idadhesion"])) {
                                        ?>
                                                <option value="<?= $cotisation['id'] ?>" <?php if ($cotisation['id'] == $cotisationModif) echo 'selected'; ?>>
                                                    <?= $cotisation['type'] . " " . $cotisation['description'] . " " . $cotisation['montant'] . " $" ?>
                                                </option>
                                            <?php
                                            } else { ?>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr class="border-b dark:border-gray-700 <?= $num % 2 === 0 ? 'bg-green-50 dark:bg-gray-600' : '' ?>">
                                ...
                                <td class="p-2"><?php echo $paiement["datePaiement"] ?></td>
                                <td class="p-2"><?php echo $paiement["nom"] . " " . $paiement["prenom"] . " " . $paiement["postnom"]  ?></td>
                                <td class="p-2"><img src="../img/<?= $paiement["photo"] ?>" alt="" class="w-11 h-11 rounded-full cursor-pointer"></td>
                                <td class="p-2"><?php echo $paiement["nomCotisation"] ?></td>
                                <td class="p-2"><?php echo $paiement["montant_du"] ?></td>
                                <td class="p-2"><?php echo $paiement["montantPaye"] ?></td>
                                <td class="p-2"><?php echo $paiement["montantRestant"] ?></td>
                                <td class="flex gap-2 p-2">
                                    <a href="paiement.php?idPaiement=<?= $paiement["id"] ?>" class="text-blue-600 hover:text-blue-800" title="Modifier">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <button onclick="openDeleteModal('<?= $paiement['id'] ?>')" title="Supprimer" class="text-red-600 hover:text-red-800">
                                        <i class="bi bi-trash"></i>
                                    </button>
                                     <a href="paiement-print.php?idPaiement=<?= $paiement["id"] ?>" class="text-blue-600 hover:text-blue-800" title="Modifier">
    ...
    <div id="modalAdhesion" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white dark:bg-gray-800 p-6 rounded shadow-md w-full max-w-2xl">
            <h2 class="text-xl font-bold mb-4 text-center">Reglement Frais de cotisation</h2>
            <form action="<?= $Action ?>" method="POST">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block mb-1">Montant<span class="text-red-600"> *</span></label>
                        <input type="text" name="montant" required class="w-full px-4 py-2 border rounded dark:bg-gray-700 dark:text-white" placeholder="Enter le postnom" <?php if (isset($_GET['idadhesion'])) { ?>value="<?= $tab['montant'] ?>" <?php } ?> />
                    </div>
                </div>
                <div class="flex justify-end gap-2">
